...
export class AdminDashboard implements OnInit {
  ...
  private loadStats(): void {
    ...
    forkJoin({
      ...
      orgAdmins: this.userService.searchUsers({
        page: 0,
        size: 1,
        role: UserRole.ORGANIZER_ADMIN,
      }),
    }).subscribe({
      next: ({ users, organizations, events, activeEvents, disabledUsers, orgAdmins }) => {
        ...
        this.orgAdmins.set(orgAdmins.totalElements);
  ...
  private loadRoleDistribution(): void {
    ...
    forkJoin({
      ...
      orgUsers: this.userService.searchUsers({
        page: 0,
        size: 1,
        role: UserRole.ORGANIZER_USER,
      }),
      distributors: this.userService.searchUsers({
        page: 0,
        size: 1,
        role: UserRole.DISTRIBUTOR,
      }),
    }).subscribe({
      next: ({ orgAdmins, orgUsers, distributors }) => {
        const ds = getComputedStyle(document.documentElement);
        this.roleChartData.set({
          labels: ['Org Admins', 'Org Users', 'Distributors'],
          datasets: [
            {
              data: [orgAdmins.totalElements, orgUsers.totalElements, distributors.totalElements],
              backgroundColor: [
