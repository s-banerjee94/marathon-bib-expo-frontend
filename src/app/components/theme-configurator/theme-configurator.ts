...
@Component({
  ...
  host: {
    class:
      'hidden absolute top-[52px] right-0 w-72 p-4 border rounded origin-top shadow-lg z-50 bg-surface-0 dark:bg-surface-900',
  },
})
export class ThemeConfigurator implements OnInit {
  ...
  getPresetExt() {
    const color: SurfacePalette | undefined = this.primaryColors().find(
  ...
  updateColors(event: Event, type: string, color: SurfacePalette) {
    if (type === 'primary') {
      this.layoutService.setPrimary(color.name);
    } else if (type === 'surface') {
      this.layoutService.setSurface(color.name);
    }
    this.applyTheme(type, color);
    event.stopPropagation();
  }

  applyTheme(type: string, color: SurfacePalette) {
    if (type === 'primary') {
